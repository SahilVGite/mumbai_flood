...
const ReportedFloodsModel = () => {
  ...
  return (
    <>
      {/* Header */}
      <ModelHeader title="Reported Floods" Icon={FloodReportIcon} />
      ...
      <div className="mt-2">
        <ul className="flex items-stretch">
          {TABS.map((tab) => (
            <li
              key={tab.name}
              className={`flex gap-4 items-center w-1/2 justify-center p-2.5 transition-all 
              text-[#A2A2AA] font-medium text-[12px] md:text-[14px] 
              [@media(min-width:1700px)]:text-[16px] 
              rounded-lg md:rounded-b-none md:rounded-t-2xl cursor-pointer 
              ${activeTab === tab.name ? "bg-black" : ""}`}
              onClick={() => {
        ...
        <ul
          className={`md:bg-(--black-50) rounded-[8px] py-4 md:px-3.5 transition-all 
          ${activeTab === "Flood Report Form" ? "rounded-tl-none" : "rounded-tr-none"}`}
        >
          {!selectedReport ? (
            <>
              {activeTab === "Flood Report Form" &&
                reportedFloodData.map((report, index) => (
                  <li
                    ...
                    className="mb-4 p-2 md:p-4 bg-black 
                    md:bg-[rgba(162,162,162,0.15)] 
                    rounded-[8px] flex items-center justify-between gap-2.5 
                    last:mb-0 hover:bg-[rgba(162,162,162,0.25)] 
                    transition-all cursor-pointer"
                  >
                    <p className="font-medium text-[12px] md:text-[16px] 
                    [@media(min-width:1700px)]:text-lg text-[#A2A2A2] w-fit">
                      {report.user}
                    </p>

                    <p className="font-medium text-[12px] 
                    [@media(min-width:1700px)]:text-[14px] 
                    text-[#A2A2A2] w-fit flex items-center">
                    ...
                    <p className="line-clamp-1 font-medium text-[10px] md:text-[12px] 
                    [@media(min-width:1700px)]:text-[14px] text-white max-w-[25%]">
                      {report.location.area}
                    </p>

                    <p className="text-[12px] [@media(min-width:1700px)]:text-[14px] 
                    w-fit whitespace-nowrap flex items-center font-medium">
                      <span className="flex items-center gap-1 border-r border-white 
                      pr-2 mr-2 text-[#32C545]">
                        {report.votes.upvote}
              ...
              {activeTab === "Tweets" &&
                twittedData.map((tweet, index) => (
                  <li
                    ...
                    className="mb-4 p-2 md:p-4 bg-black 
                    md:bg-[rgba(162,162,162,0.15)] 
                    rounded-[8px] last:mb-0"
                  >
                    <div className="flex items-center gap-10">
                      <span className="font-medium text-[12px] md:text-[16px] 
                      [@media(min-width:1700px)]:text-lg text-[#A2A2A2]">
                        {tweet.user}
                      </span>
                    ...
                    <div className="text-[#A2A2A2] font-medium text-[12px] 
                    [@media(min-width:1700px)]:text-[14px] mt-4">
                      <p>{tweet.twitter}</p>
                      <p>{tweet.message}</p>
                      <p>{tweet.tags.join(" ")}</p>
                    </div>
            ...
            <div className="bg-black md:bg-[rgba(162,162,162,0.15)] rounded-[8px] p-3 md:p-6">
              ...
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-medium text-[12px] md:text-[16px] 
                [@media(min-width:1700px)]:text-lg text-white">
                  {selectedReport.user}
                </h3>

                <p className="text-[#A2A2A2] text-[12px] 
                [@media(min-width:1700px)]:text-[14px] font-medium">
              ...
              <p className="text-white mb-4 font-medium text-[12px] 
              [@media(min-width:1700px)]:text-[14px]">
              ...
              <div className="text-white italic mb-10 text-[12px] 
              [@media(min-width:1700px)]:text-[14px]">
                “{selectedReport.message}”
              </div>

              <div className="flex justify-center font-medium gap-8 text-[12px] 
              [@media(min-width:1700px)]:text-[14px]">
                <button className="bg-[#FF2463] text-white px-6 py-4 min-w-[40%] 
                rounded-lg flex items-center justify-center gap-2 cursor-pointer">
                  <DownVote className="w-4 h-4" strokeColor="#ffffff" />
                  Down vote
                </button>
                <button className="bg-[#32C545] text-white px-6 py-4 min-w-[40%] 
                rounded-lg flex items-center justify-center gap-2 cursor-pointer">
